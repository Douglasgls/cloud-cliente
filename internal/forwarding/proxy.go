...
func formatListenError(localPort int, err error) error {
	...
	errStr := err.Error()
	if strings.Contains(errStr, "address already in use") || strings.Contains(errStr, "only one usage of each socket address") || strings.Contains(errStr, "EADDRINUSE") || strings.Contains(errStr, "endereço de soquete") || strings.Contains(errStr, "uma utilização de cada") {
